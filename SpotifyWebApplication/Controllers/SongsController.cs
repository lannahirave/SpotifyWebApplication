...
public class SongsController : Controller
{
    ...
    {
        if (id is null) return RedirectToPage("Index", "Playlists");

        var playlist = await _context.Playlists.FirstOrDefaultAsync(a => a.Id == id);
        if (playlist is null) return RedirectToPage("Index", "Playlists");
    ...
    {
        if (playlistId is null) return NotFound();
    ...
    public async Task<IActionResult> AddPlaylistSong(int albumId,
        [Bind("Id, PlaylistId, SongId")] PlaylistsSong playsong)
    {
    ...
    {
        if (id != songEdit.Id) return NotFound();
